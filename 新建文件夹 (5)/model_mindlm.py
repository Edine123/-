...
class MindLM(nn.Module):
    ...
    def forward(self, x_brain=None, y_brain=None, x_text=None, y_text=None, attention_mask=None, Y=None):
        ...
        if x_brain is not None:
            if self.tune_tokenizer:
                ...
                with torch.no_grad():
                    ...
                    if x_brain.shape[1] > 160:
                        # 交换维度：从 (B, 2560, H) 变成 (B, H, 2560)
                        x_brain = x_brain.transpose(1, 2)
                        # 在时序/序列维度上进行自适应均值池化，强制将 2560 压成 160
                        x_brain = torch.nn.functional.adaptive_avg_pool1d(x_brain, 160)
                        # 还原维度：从 (B, H, 160) 变回 (B, 160, H)
                        x_brain = x_brain.transpose(1, 2)
        ...
        # # ======================================================================
        # 🛡️ 【多模态包装层·解包与三元组安全重组】 🛡️
        # ======================================================================
        # 1. 物理安全调用，防止底层因为意外返回单体或 None 导致直接炸进程
        llm_outputs = self.llm(x_brain, y_brain, x_text, y_text, attention_mask, Y=Y)
        
        # 2. 智能兼容底层返回的各种元组长度（有些分支返回 2 个，有些返回 3 个）
        if isinstance(llm_outputs, tuple):
            logits = llm_outputs[0] if len(llm_outputs) > 0 else None
            loss = llm_outputs[1] if len(llm_outputs) > 1 else torch.tensor(0.0, device=x_brain.device, requires_grad=True)
            accuracy = llm_outputs[2] if len(llm_outputs) > 2 else 0.0
        else:
            # 极度空指针保底
            logits = llm_outputs
            loss = torch.tensor(0.0, device=x_brain.device, requires_grad=True)
            accuracy = 0.0

        # 3. 🎯 【决定性对齐】：由于最外层 train_instruction.py 接收的格式是：
        # loss1, log1, logits = model(...)
        # 我们必须严格按照这个顺序（Loss, Log字典, Logits）把数据打包装箱打包送出去！
        
        # 组装用于 TensorBoard/Wandb 监控的日志字典
        log_dict = {
            'train/total_loss': loss.item() if torch.is_tensor(loss) else float(loss),
            'train/fmri_loss': loss.item() if torch.is_tensor(loss) else float(loss),
            'train/accuracy': accuracy if isinstance(accuracy, (int, float)) else 0.0,  # 💡 补上缺失的键
            'train/fmri_acc': accuracy if isinstance(accuracy, (int, float)) else 0.0,
            'train/loss': loss.item() if torch.is_tensor(loss) else float(loss)
        }
        
        # 🌟 完美的对齐返回：[损失张量, 日志字典, 预测Logits]
        return loss, log_dict, logits
        # ======================================================================
        log = {}
    ...
    def generate(self, x_brain, x_text, attention_mask=None, max_new_tokens=10, temperature=0.8, top_k=20, top_p=0.8, text_gen=False, accelerator=None, **kwargs):
        ...
        # print("======== GENERATE ========")
        # print("x_brain shape =", x_brain.shape if x_brain is not None else None)
        # print("x_text shape =", x_text.shape if x_text is not None else None)
